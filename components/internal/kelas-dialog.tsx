...
import { Plus, X, Star, School as SchoolIcon, Users2, GraduationCap, Info, ChevronDown } from "lucide-react";
...
type FormState = { name: string; schedule: string; capacity: string; program_id: string; school_id: string; trainers: TrainerRow[] };
...
export function KelasDialog({ open, kelas, onClose }: { open: boolean; kelas: KelasForDialog | null; onClose: () => void }) {
    ...
    const isEdit = !!kelas;
    const [form, setForm] = useState<FormState>({ name: "", schedule: "", capacity: "", program_id: "", school_id: "", trainers: [] });
