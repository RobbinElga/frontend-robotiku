...
export default function TandaTanganPage() {
    ...
    const { data } = useQuery({ queryKey: ["my-ttd"], queryFn: async () => (await api.get<ApiEnvelope<{ url: string | null }>>("/profil/tanda-tangan")).data.data });
    ...
    const save = useMutation({
        mutationFn: async () => { const fd = new FormData(); fd.append("image", file!); return api.post("/profil/tanda-tangan", fd); },
        onSuccess: () => { setFile(null); setSaved(true); setTimeout(() => setSaved(false), 2500); qc.invalidateQueries({ queryKey: ["my-ttd"] }); },
    });

    const currentPath = data?.url?.replace("/api/v1/media/", "") ?? null;
            ...
            <Card className="max-w-2xl p-6">
                ...
                <div className="flex flex-col gap-4 sm:flex-row">
                    <div className="flex h-32 w-56 shrink-0 items-center justify-center overflow-hidden rounded-xl border-2 border-dashed bg-white">
                        {preview ? <img src={preview} alt="ttd" className="h-full w-full object-contain p-2" /> : currentPath ? <AuthImage path={currentPath} alt="ttd" className="h-full w-full object-contain p-2" /> : <span className="text-xs text-muted-foreground">Belum ada TTD</span>}
                    </div>
